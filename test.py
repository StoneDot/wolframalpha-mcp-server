...
def test_unit_conversion_prompt():
    ...
    for i, test_case in enumerate(test_cases, 1):
        print(f"Test {i}: {test_case['name']}")
        print(
            f"Converting {test_case['value']} {test_case['from_unit']} to {test_case['to_unit']}"
        )
...
async def test_completion_functionality():
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        try:
            ...
            if (
                test_case.get("expected_result") is None
                and test_case["ref"].name != "unit_conversion"
            ):
                if result is None:
                ...
                # This is a unit conversion test - should return completion results
                if result is None:
                    ...
                    if "expected_min_results" in test_case:
                        ...
                        if len(values) >= min_expected:
                            ...
                            print(
                                f"✗ Expected at least {min_expected} results, got {len(values)}"
                            )
